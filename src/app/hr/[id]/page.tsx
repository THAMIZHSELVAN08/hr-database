...
import { getTeamDataByEmail } from '@/app/add/components/TeamData';
...
// Users who can transfer contacts between members
const TRANSFER_EXCEPTIONS = ['2023ee0724@svce.ac.in', '2023cs0051@svce.ac.in'];
...
export default function HREditPage({ params }: { params: { id: string } }) {
      ...
      try {
        ...
        if (userRes.ok) {
          ...
          if (mounted) {
            ...
            setCanTransfer(TRANSFER_EXCEPTIONS.includes(userData.email));
          }
        }

        const res = await fetch(`/api/hr/${params.id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/hr/${params.id}`);
    } catch (err) {
    ...
    <div className="p-6">
      <div className="max-w-5xl mx-auto">
        <div className="bg-[#111111] rounded-xl border border-gray-800 p-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">Edit HR Record</h1>
            {canTransfer && (
              <div className="mt-4 px-4 py-3 bg-amber-600/20 border border-amber-500/30 rounded-lg">
                <p className="text-amber-300 text-sm font-semibold">
                  🔓 Transfer Mode Active: You can edit member and incharge fields to transfer this contact
                </p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-300 mb-2 items-center gap-2">
                  Member*
                  {canTransfer && (
                    <span className="text-xs px-2 py-0.5 bg-amber-600/20 border border-amber-500/30 rounded text-amber-300">
                      EDITABLE
                    </span>
                  )}
                </label>
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full px-3 py-2 border rounded text-white focus:outline-none focus:ring-2 ${
                    canTransfer
                      ? 'bg-gray-900 border-gray-700 focus:ring-amber-500'
